...
app.get("/oembed", async (req, res) => {
  const { url } = req.query;
  if (!url) return res.status(400).json({ error: "Missing tweet URL" });
  ...
  try {
    ...
    if (!response.ok) return res.status(502).json({ error: "Twitter oEmbed request failed" });
    const data = await response.json();
    const match = data.html?.match(/<p[^>]*>([\s\S]*?)<\/p>/i);
    if (!match) return res.status(502).json({ error: "Could not parse tweet text" });
...
app.post("/analyze", async (req, res) => {
  ...
  if (!tweetText) return res.status(400).json({ error: "Missing tweetText" });
  ...
  try {
    ...
    if (data.error) {
      return res.status(502).json({ error: "Anthropic API error", detail: data.error });
    }
    ...
    res.status(500).json({ error: "Failed to analyze bet", detail: err.message });
  }
});
